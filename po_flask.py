...
def outstanding_summary():
    ...
    with get_db() as conn:
        ...
        overdue_count = 0
        advance_paid_usd = 0.0

        for sup in suppliers:
            sid = sup["id"]
            cursor.execute("""
                SELECT dr_cr, amount_usd, amount_inr, entry_date, entry_type
                FROM supplier_ledger_entries
            ...
            # Advance payments: PAYMENT entries where no INVOICE existed yet for that PO
            adv = sum(e["amount_usd"] for e in entries
                      if e["entry_type"] == "PAYMENT" and e["dr_cr"] == "DR")
            advance_paid_usd += adv
    ...
    return jsonify({
        ...
        "advance_paid_usd": round(advance_paid_usd, 2),
    })
...
def get_pos():
    ...
    with get_db() as conn:
        ...
        for row in cursor.fetchall():
            ...
            # Fetch linked shipment info
            cursor2 = conn.cursor()
            cursor2.execute("""
                SELECT s.id, s.description, s.status, s.expected_arrival
                FROM shipments s
                JOIN shipment_po_link spl ON spl.shipment_id = s.id
                WHERE spl.po_id = ? AND s.deleted_at IS NULL
            """, (po["id"],))
            ship_row = cursor2.fetchone()
            po["shipment"] = dict(ship_row) if ship_row else None
...
def add_customer_ledger_entry(cid):
    ...
    amount_usd = _safe_float(req.get("amount_usd"), round(amount_inr / usd_rate, 2))
    ...
    with get_db() as conn:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            eid, cid, entry_type, entry_date,
            req.get("ref_number", ""), req.get("description", ""),
            amount_inr, amount_usd, usd_rate, dr_cr,
            req.get("payment_mode", ""), req.get("bank_ref", ""),
            due_date, req.get("notes", ""), req.get("created_by", "User")
        ))
...
def delete_customer_ledger_entry(eid):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM customer_ledger_entries WHERE id = ? AND deleted_at IS NULL", (eid,))
        if not cursor.fetchone():
            return jsonify({"error": "Ledger entry not found"}), 404
    ...
    with get_db() as conn:
        ...
        for c in customers:
            ...
            if ageing["total_due"] > 0:
                ...
                for k in totals: totals[k] += ageing[k]
