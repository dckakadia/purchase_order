...
def import_data():
    ...
    with get_db() as conn:
        try:
            ...
            # ❌ ROLLBACK on error (automatic with context manager via get_db)
...
def upload_payment_proof(pid):
    ...
    # ── Validate request fields before touching DB or disk ───────────────────
    if "file" not in request.files:
        return jsonify({"error": "No file provided"}), 400
    file = request.files["file"]
    if not file.filename:
    ...
    # ── Check lock status and fetch PO number (read-only pass) ───────────────
    with get_db() as conn:
        _migrate_payment_json(pid, conn)
        cursor = conn.cursor()
        cursor.execute(
    ...
    # ── Save file to disk (outside DB transaction to avoid orphan on DB error) 
    ...
    # ── Persist metadata; clean up file if DB write fails ────────────────────
    uploaded_at = datetime.now().strftime("%Y-%m-%d %H:%M")
    try:
        with get_db() as conn:
            cursor = conn.cursor()
        ...
        # Remove the file we just saved so disk and DB stay in sync
        try:
            os.remove(dest_path)
        except OSError:
            pass
        return jsonify({"error": f"Failed to record payment proof: {str(e)}"}), 500
